...
class SelfImitation(object):
    ...
    def add_episode(self, trajectory):
        ...
        for (ob, action, reward) in trajectory:
            ...
            rewards.append(self.fn_reward(reward)) if self.fn_reward is not None else rewards.append(reward)
            dones.append(False)
    ...
    def build_loss_op(self):
        ...
        #self.vf_loss = tf.reduce_sum(self.W *tf.stop_gradient(delta)* tf.stop_gradient(delta)) / self.num_samples
    ...
    def _train(self, sess, lr):
        ...
        loss, adv, mean_adv, samples, nlogp, _ = sess.run(
                [self.loss, self.adv, self.mean_adv, self.num_valid_samples,
                    self.neg_log_p, self.train_op], td_map)       
