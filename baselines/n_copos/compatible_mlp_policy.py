...
class CompatibleMlpPolicy(object):
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, init_std=1.0, gaussian_fixed_var=True):
        ...
        action = utils.get_placeholder(name="act", dtype=tf.float32, shape=[sequence_length] + list(ac_space.shape))
        logprob = self.pd.logp(action)
        self._logprob = utils.function([action, self.ob], [logprob])
    ...
    def logprob(self, action, ob):
        logprob = self._logprob(action[None], ob[None])
        return logprob[0]
