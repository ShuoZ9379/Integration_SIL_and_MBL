...
def main():  
    ...
    parser.add_argument('--expname', type=str, default='EXP_ON_2_lr1')
...
#           '','--sil_update=2 --sil_loss=0.001','']
    ...
    for i in range(args.st_seed, args.st_seed+args.seeds):
        for j in range(1):
            os.system("python /work/scratch/sz52cacy/Desktop/carla_sample_efficient/algos/"+algo_names[j]+"/run.py --alg="+algo_names[j]+" --num_timestep="
                      +args.num_timesteps+" --seed="+str(i)+" --env="+args.env+" --log_path=/work/scratch/sz52cacy/Desktop/logs/"+args.expname+"/"
