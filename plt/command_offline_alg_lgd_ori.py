            ...
            if args.time=='24:00:00':
                if args.num_timesteps=='1e6':
                    os.system("python /work/scratch/sz52cacy/Desktop/carla_sample_efficient/algos/"+algo_names[j]+"/run.py --alg="+algo_names[j]+" --num_timesteps="
                              +args.num_timesteps+" --seed="+str(i)+" --env="+args.env+" --log_path=/work/scratch/sz52cacy/Desktop/logs/EXP_OFF_24_1M_V0/"
                    ...
                    os.system("python /work/scratch/sz52cacy/Desktop/carla_sample_efficient/algos/"+algo_names[j]+"/run.py --alg="+algo_names[j]+" --num_timesteps="
                              +args.num_timesteps+" --seed="+str(i)+" --env="+args.env+" --log_path=/work/scratch/sz52cacy/Desktop/logs/EXP_OFF_24_5M_V0/"
                              +args.env+"/"+legend_names[j]+"-"+str(i)+' '+argus[j])
                    #results = pu.load_results('~/Desktop/logs/EXP_OFF_24_5M_V0/'+args.env+"/"+dct[algo_names[j]]+"-"+str(i))
                            
            else:
                os.system("python /work/scratch/sz52cacy/Desktop/carla_sample_efficient/algos/"+algo_names[j]+"/run.py --alg="+algo_names[j]+" --num_timesteps="
                          +args.num_timesteps+" --seed="+str(i)+" --env="+args.env+" --log_path=/work/scratch/sz52cacy/Desktop/logs/EXP_OFF_120_V0/"
                          +args.env+"/"+legend_names[j]+"-"+str(i)+' '+argus[j])
