...
def learn(*,
        ...
        total_timesteps,
        timesteps_per_batch, sil_update, sil_loss,# what to train on
        max_kl=0.001,
        cg_iters=10,
        gamma=0.99,
...
################### use_ent_adjust======> adj_std_scale????????pi action sample
    ...
    for n in num_samples:
        for h in horizon:
            for l in mbl_lamb:
                for e in num_elites:                     
                    if 'mbmf' in eval_targs: all_eval_descs.append(('MeanRew', 'MBL_TRPO_SIL', make_mbmf_pi(n, h, e, l)))
    ...
    if 'mf' in eval_targs: all_eval_descs.append(('MeanRew', 'TRPO_SIL', Policy(step=_mf_pi, reset=None)))
