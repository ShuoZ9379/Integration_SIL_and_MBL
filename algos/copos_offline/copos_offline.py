...
################### use_ent_adjust======> adj_std_scale????????pi action sample
    ...
    for n in num_samples:
        for h in horizon:
            for l in mbl_lamb:
                for e in num_elites:                     
                    if 'mbmf' in eval_targs: all_eval_descs.append(('MeanRew', 'MBL_COPOS', make_mbmf_pi(n, h, e, l)))
    ...
    if 'mf' in eval_targs: all_eval_descs.append(('MeanRew', 'COPOS', Policy(step=_mf_pi, reset=None)))
